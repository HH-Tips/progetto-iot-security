...
void authorize_ip(const char *ip) {
  ...
  char cmd[256];
  snprintf(cmd, sizeof(cmd), "iptables -t nat -I CAPTIVE 1 -s %s -j RETURN",
           ip);
  system(cmd);
}
...
void setup_iptables(const char *router_ip) {
  ...
  snprintf(cmd, sizeof(cmd),
           "iptables -t nat -A CAPTIVE -p udp --dport 53 -j DNAT --to %s:53",
           router_ip);
  system(cmd);
  snprintf(cmd, sizeof(cmd),
           "iptables -t nat -A CAPTIVE -p tcp --dport 53 -j DNAT --to %s:53",
           router_ip);
  system(cmd);
...
int main(void) {
  ...
  /* Ferma dnsd già in esecuzione (lascia httpd attivo sulla porta 80) */
  system("killall -9 dnsd 2>/dev/null");

  /* ── Aspetta che br0 abbia un IP (max 30 secondi) ───── */
  char router_ip[64] = "192.168.0.1";
  for (int i = 0; i < 30; i++) {
    if (get_router_ip(router_ip, sizeof(router_ip)))
      break;
    sleep(1);
  }
  ...
  char dns_cmd[256];
  snprintf(dns_cmd, sizeof(dns_cmd), "dnsd -c /tmp/dnsd.conf -i %s &",
           router_ip);
  system(dns_cmd);
  ...
  while (1) {
    ...
    if (ip_s)
      strncpy(client_ip, ip_s, sizeof(client_ip) - 1);
    ...
    if (qm)
      strncpy(path, fullpath, qm - fullpath);
    else
    ...
    if (strcmp(path, "/connect") == 0 && strcmp(method, "POST") == 0) {
      ...
      const char *ok = "<!DOCTYPE html><html><head>"
