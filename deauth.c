 ...
 */
static void build_deauth_frame(unsigned char *buf,
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 5) {
        ...
        if (num_packets <= 0) {
            fprintf(stderr, "Errore: numero di pacchetti non valido: %s\n", argv[4]);
            return EXIT_FAILURE;
        }
    }
    ...
    for (int i = 0; i < num_packets; i++) {
        ...
        build_deauth_frame(frame, bssid, client, REASON_UNSPECIFIED, (unsigned short)i);
